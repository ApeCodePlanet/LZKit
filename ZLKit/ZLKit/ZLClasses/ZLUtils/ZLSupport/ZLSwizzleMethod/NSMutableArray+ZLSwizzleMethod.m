//
//  NSMutableArray+ZLSwizzleMethod.m
//  ZLKit
//
//  Created by Sunny Leong on 2021/1/6.
//

#import "NSMutableArray+ZLSwizzleMethod.h"

@implementation NSMutableArray (ZLSwizzleMethod)
...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        ...
        [objc_getClass("__NSArrayM") SystemSelector:@selector(objectAtIndex:) swizzledSelector:@selector(kl_objectAtIndex:) error:nil];
        // 交换NSMutableArray中的方法
        [objc_getClass("__NSArrayM") SystemSelector:@selector(objectAtIndexedSubscript:) swizzledSelector:@selector(kl_objectAtIndexedSubscript:) error:nil];
    });
}
...
- (id)kl_objectAtIndex:(NSUInteger)index {
    if (index < self.count) {
        ...
        NSLog(@"NSMutableArray 数组发生越界，已经处理完成。越界下标：%lu，数组大小：%lu", (unsigned long)index, (unsigned long)self.count);
        return nil;
    }
}
